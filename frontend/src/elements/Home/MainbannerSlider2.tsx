import { useEffect, useRef, useState } from "react";
import Slider from "react-slick";
import { Link } from "react-router-dom";
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";

import { MainSwiperData, MainSwiperData2 } from '../../constant/Alldata';
import api from '../../lib/api';
import { useCurrency } from '../../context/CurrencyContext';

interface BannerSlide {
  _id: string;
  title: string;
  subtitle?: string;
  price?: string;
  image?: string;
  productLink?: string;
  buttonText?: string;
  sortOrder: number;
  isActive: boolean;
}

const MainBannerSlider2 = () => {
    const { formatPrice } = useCurrency();
    const mainSliderRef = useRef<Slider | null>(null);
    const thumbSliderRef = useRef<Slider | null>(null);
    const [nav1, setNav1] = useState<Slider | undefined>(undefined);
    const [nav2, setNav2] = useState<Slider | undefined>(undefined);
    const [slides, setSlides] = useState<BannerSlide[]>([]);
    const [loaded, setLoaded] = useState(false);

    useEffect(() => {
      setNav1(mainSliderRef.current || undefined);
      setNav2(thumbSliderRef.current || undefined);
    }, [loaded]);

    useEffect(() => {
      api.get('/banners')
        .then(({ data }) => {
          if (Array.isArray(data) && data.length > 0) setSlides(data);
        })
        .catch(() => {})
        .finally(() => setLoaded(true));
    }, []);

    const mainSettings = {
        slidesToShow: 1,
        slidesToScroll: 1,
        arrows: false,
        fade: false,
        infinite: true,
        asNavFor: nav2,
    };

    const thumbSettings = {
        slidesToShow: 2,
        slidesToScroll: 1,
        dots: false,
        centerMode: false,
        infinite: true,
        focusOnSelect: true,
        asNavFor: nav1,
    };

    // Use API data if available, otherwise fall back to static Alldata
    const textSlides = slides.length > 0
      ? slides.map(s => ({ title: s.title, price: s.price || '', productLink: s.productLink || '/product-default', buttonText: s.buttonText || 'View Detail' }))
      : MainSwiperData.map(s => ({ title: s.title, price: String(s.price), productLink: '/product-default', buttonText: 'View Detail' }));

    const imageSlides = slides.length > 0
      ? slides.map(s => ({ image: s.image || '', name: s.title }))
      : MainSwiperData2;

  return (
    <div className="row main-slide">
      <div className="col-lg-6">
        <Slider ref={mainSliderRef} {...mainSettings} className="slider-main">
            {textSlides.map((item, index) => (
                <div className="content-info" key={index}>
                    <h1 className="title">{item.title}</h1>
                    <div className="swiper-meta-items">
                        <div className="meta-content">
                            <span className="price-name">Price</span>
                            <span className="price-num d-inline-block">{formatPrice(item.price)}</span>
                        </div>
                    </div>
                    <div className="content-btn m-b30">
                        {/* <Link to="/shop-wishlist" className="btn btn-secondary me-xl-3 me-2 btnhover20">ADD TO CART</Link> */}
                        <Link to={item.productLink} className="btn btn-outline-secondary btnhover20">{item.buttonText}</Link>
                    </div>
                </div>
            ))}
        </Slider>
      </div>
      <div className="col-lg-6">
        <Slider ref={thumbSliderRef} {...thumbSettings} className="slider-thumbs">
          {imageSlides.map((item, i) => (
            <div className="banner-media" key={i} data-name={item.name}>
                <div className="img-preview">
                    <img src={item.image} alt="banner-media" />
                </div>
            </div>
          ))}
        </Slider>
      </div>
    </div>
  );
};
export default MainBannerSlider2;
