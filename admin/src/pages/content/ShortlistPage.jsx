...
const DEFAULT_CARD = { image: '', name: '', saleTitle: 'up to 79% off', price: '80', originalPrice: '95', link: '/shop', showBadge: false }
...
export function ShortlistPage() {
  ...
  useEffect(() => {
    api.get('/shortlist')
      .then(({ data }) => setForm({ ...DEFAULT, ...data, cards: data.cards?.length ? data.cards : [] }))
  ...
  const addCard = () => setForm(f => ({ ...f, cards: [...f.cards, { ...DEFAULT_CARD }] }))
  ...
  const handleBannerUpload = async (e) => {
    ...
    finally { setUploading(u => ({ ...u, banner: false })) }
  }

  const handleCardUpload = (key) => async (e) => {
    const file = e.target.files?.[0]; if (!file) return
  ...
  const handleMediaSelect = (file) => {
    if (mediaTarget === 'banner') set('bannerImage', file.url)
    else {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Shortlist Section</h1>
          <p className="text-sm text-gray-500 mt-0.5">Manage the "Recent Additions to Your Shortlist" section on the home page</p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
        <h2 className="text-sm font-semibold text-gray-800 border-b border-gray-100 pb-2">Banner</h2>
        ...
        <ImagePicker
          ...
          onOpenMedia={() => setMediaTarget('banner')}
        />
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={form.bannerTitle} onChange={e => set('bannerTitle', e.target.value)} placeholder="Recent Additions to Your Shortlist" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Animation Text</label>
            <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={form.animationText} onChange={e => set('animationText', e.target.value)} placeholder="Shortlist" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Button Text</label>
            <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={form.btnText} onChange={e => set('btnText', e.target.value)} placeholder="Shop Now" />
          </div>
          <div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-gray-100 pb-2">
          <h2 className="text-sm font-semibold text-gray-800">Product Cards</h2>
          <button onClick={addCard} className="flex items-center gap-1 bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs hover:bg-blue-700">
            <Plus size={13} />Add Card
          </button>
        </div>

        {form.cards.length === 0 && (
          <p className="text-sm text-gray-400 text-center py-4">No cards yet. Click "Add Card" to get started.</p>
        )}
              ...
              <div key={i} className="border border-gray-100 rounded-xl p-4 space-y-3 bg-gray-50/50">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => moveCard(i, -1)} disabled={i === 0} className="p-1 rounded hover:bg-gray-100 disabled:opacity-30 text-gray-500"><ChevronUp size={14} /></button>
                    <button onClick={() => moveCard(i, 1)} disabled={i === form.cards.length - 1} className="p-1 rounded hover:bg-gray-100 disabled:opacity-30 text-gray-500"><ChevronDown size={14} /></button>
                    <button onClick={() => removeCard(i)} className="p-1 rounded hover:bg-red-50 text-red-400"><Trash2 size={14} /></button>
                  </div>
                </div>
                ...
                <ImagePicker
                  ...
                  onUpload={handleCardUpload(`card-${i}`)}
                ...
                <label className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer">
                  <input type="checkbox" checked={!!card.showBadge} onChange={e => setCard(i, 'showBadge', e.target.checked)}
