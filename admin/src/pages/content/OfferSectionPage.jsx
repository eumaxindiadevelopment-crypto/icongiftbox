...
const HEADING_STYLES = [
  { value: 'product-name', label: 'Product Name (simple heading)' },
  { value: 'sub-title1',   label: 'Sub Title 1 (heading + year span)' },
  { value: 'sub-title2',   label: 'Sub Title 2 (heading + bg-title span)' },
]

const DEFAULT_SLIDE = {
  backgroundImage: '',
  offerText:       '20% Off',
  headingStyle:    'product-name',
  heading:         '',
  spanText:        '',
  btnText:         'Collect Now',
  btnLink:         '/shop-list',
...
export function OfferSectionPage() {
  ...
  const moveSlide = (i, dir) => setForm(f => {
    const items = [...f.slides]; const j = i + dir
    if (j < 0 || j >= items.length) return f
    ;[items[i], items[j]] = [items[j], items[i]]
    return { ...f, slides: items }
  })
  ...
  const handleUpload = (key) => async (e) => {
    ...
    try {
      ...
      setSlide(idx, 'backgroundImage', data.url)
  ...
  const handleMediaSelect = (file) => {
    ...
    setSlide(idx, 'backgroundImage', file.url)
    setMediaTarget(null)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      await api.put('/offer-section', form)
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Offer Section Slider</h1>
          <p className="text-sm text-gray-500 mt-0.5">Manage the featured offer slider on the home page</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={form.sectionTitle} onChange={e => set('sectionTitle', e.target.value)} placeholder="Featured offer for you" />
          </div>
          <div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-gray-100 pb-2">
          <h2 className="text-sm font-semibold text-gray-800">Offer Slides</h2>
            ...
            return (
              <div key={i} className="border border-gray-100 rounded-xl p-4 space-y-3 bg-gray-50/50">
                ...
                <ImagePicker
                  ...
                  onChange={v => setSlide(i, 'backgroundImage', v)}
                  fileRef={fileRefs.current[i]}
                  onUpload={handleUpload(`slide-${i}`)}
                  uploading={uploading[`slide-${i}`]}
                  onOpenMedia={() => setMediaTarget(`slide-${i}`)}
                />

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Offer Text</label>
                    <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                      value={slide.offerText} onChange={e => setSlide(i, 'offerText', e.target.value)} placeholder="20% Off" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Heading Style</label>
                    <select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                      value={slide.headingStyle} onChange={e => setSlide(i, 'headingStyle', e.target.value)}>
                      {HEADING_STYLES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
                    </select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                      value={slide.heading} onChange={e => setSlide(i, 'heading', e.target.value)} placeholder="e.g. Luxury Bras / Summer / Swimwear" />
                  </div>
                  {slide.headingStyle !== 'product-name' && (
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">
                        {slide.headingStyle === 'sub-title1' ? 'Year (span text)' : 'Sale Span Text'}
                      </label>
                      <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                        value={slide.spanText} onChange={e => setSlide(i, 'spanText', e.target.value)}
                        placeholder={slide.headingStyle === 'sub-title1' ? '2024' : 'Sale'} />
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                      value={slide.btnText} onChange={e => setSlide(i, 'btnText', e.target.value)} placeholder="Collect Now" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Button Link</label>
                    <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                      value={slide.btnLink} onChange={e => setSlide(i, 'btnLink', e.target.value)} placeholder="/shop-list" />
                  </div>
                </div>
              </div>
            )
