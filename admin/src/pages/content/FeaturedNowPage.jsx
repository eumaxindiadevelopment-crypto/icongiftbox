...
const DEFAULT_ITEM = { image: '', name: '', saleTitle: 'Up to 40% Off', price: '80', originalPrice: '95', review: '(2k Review)', link: '/shop-list' }
...
export function FeaturedNowPage() {
  ...
  const moveItem = (i, dir) => setForm(f => {
    const items = [...f.sliderItems]; const j = i + dir
  ...
  const handleUpload = (key) => async (e) => {
    ...
    try {
      ...
      setItem(parseInt(key.split('-')[1]), 'image', data.url)
    } catch { setError('Upload failed') }
    finally { setUploading(u => ({ ...u, [key]: false })) }
  }

  const handleMediaSelect = (file) => {
    setItem(parseInt(mediaTarget.split('-')[1]), 'image', file.url)
    setMediaTarget(null)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      await api.put('/featured-now', form)
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Featured Now Section</h1>
          <p className="text-sm text-gray-500 mt-0.5">Manage the Featured Now slider on the home page</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={form.sectionTitle} onChange={e => set('sectionTitle', e.target.value)} placeholder="Featured now" />
          </div>
          <div>
              ...
              <div key={i} className="border border-gray-100 rounded-xl p-4 space-y-3 bg-gray-50/50">
                ...
                <div>
                  ...
                  <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                    value={item.name} onChange={e => setItem(i, 'name', e.target.value)} placeholder="Product name" />
                </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  <div>
                    ...
                    <input className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                      value={item.saleTitle} onChange={e => setItem(i, 'saleTitle', e.target.value)} placeholder="Up to 40% Off" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                      value={item.review} onChange={e => setItem(i, 'review', e.target.value)} placeholder="(2k Review)" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                    value={item.link} onChange={e => setItem(i, 'link', e.target.value)} placeholder="/shop-list" />
                </div>
              </div>
            )
          })}
